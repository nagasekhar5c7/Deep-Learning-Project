...
def build_M_matrix(matrix_occ, data, event_location_dict, c):
    ...
    for idx, row in tqdm(data.iterrows(), total=len(data), desc='Row'):
        #print("entered build m matrix-->" + str(idx) )
        ev_list = []
...
def build_event_matrix(combined):
    ...
    #all_codes = all_codes.extend([0,1])
...
def code2gender(gender):
    code = "M" if gender==0 else "F"
    return code
...
def build_save_event_matrix(combined_path, matrix_path, event_location_dict_path):
    ...
    filtered_combined['GENDER']  = filtered_combined.apply(lambda x: pd.array(list(code2gender(x['GENDER']))),axis=1)
